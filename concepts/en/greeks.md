...
The closed form gives $\Theta$ per year; desks quote it per calendar day ($\Theta/365$) or trading day ($\Theta/252$). A lost factor of 365 turns a $-0.02$ daily decay into an absurd $-7.94$.
:::

::: pitfall Believing a delta-hedged position is riskless
Delta hedging removes the first-order exposure only; what remains is $\tfrac12\Gamma S^2(\sigma_{\text{real}}^2 - \sigma_{\text{impl}}^2)\,dt$ plus vega exposure to implied-vol moves. Short gamma loses on every large move: the classic blow-up.
:::

::: pitfall Quoting vega in the wrong units
$\nu = 27.9$ means $27.9$ per unit of $\sigma$, i.e. per 100 volatility points; per point it is $0.279$. The wrong one overstates vega risk by a factor of 100.
:::

::: pitfall Netting Greeks across different underlyings or expiries
A long delta in one stock does not hedge a short delta in another, and long 2-year vega does not hedge short 1-month vega: implied vols of different expiries move differently. Aggregate only within a bucket where the risk factor is common.
:::

## 30-Second Revision

Greeks are the partial derivatives of the option price. Call: $\Delta = N(d_1)$, $\Gamma = \varphi(d_1)/(S\sigma\sqrt{T})$, $\nu = S\varphi(d_1)\sqrt{T}$, $\Theta = -S\varphi(d_1)\sigma/(2\sqrt{T}) - rKe^{-rT}N(d_2)$, $\rho = KTe^{-rT}N(d_2)$; the put shares $\Gamma$ and $\nu$ and has delta $N(d_1) - 1$. The PDE says $\Theta + \tfrac12\sigma^2S^2\Gamma + rS\Delta - rV = 0$, so long gamma costs theta. A delta-hedged position earns $\tfrac12\Gamma S^2(\sigma_{\text{real}}^2 - \sigma_{\text{impl}}^2)\,dt$: a bet on realised versus implied variance, weighted by where the gamma sits. Desks quote delta in cash, gamma per 1 %, vega per point, theta per day.
...
Parity gives $\Delta_P = N(d_1) - 1$, the **same** $\Gamma$ and $\nu$, $\Theta_P = -\dfrac{S\varphi(d_1)\sigma}{2\sqrt{T}} + rKe^{-rT}N(-d_2)$, $\rho_P = -KTe^{-rT}N(-d_2)$. Two identities: $\nu = \sigma T S^2\,\Gamma$, and the PDE read between Greeks:
...
**Higher order.** The most used cross sensitivities are
...
Vanna says how the hedge moves with volatility; volga is the convexity that makes out-of-the-money options benefit from vol-of-vol. Charm ($\partial\Delta/\partial t$) and speed ($\partial\Gamma/\partial S$) also appear on desk reports.

**Desk units.** Delta in shares or cash ($\Delta \cdot S$); gamma as the change in cash delta for a 1 % move ($\Gamma S^2/100$); vega per volatility point ($\nu/100$); theta per calendar day ($\Theta/365$).
...
**Delta.** $\partial_S C = N(d_1) + S\varphi(d_1)\,\partial_S d_1 - Ke^{-rT}\varphi(d_2)\,\partial_S d_2$; since $\partial_S d_1 = \partial_S d_2 = 1/(S\sigma\sqrt{T})$, the identity cancels the last two terms: $\Delta = N(d_1)$. Then $\Gamma = \partial_S N(d_1) = \varphi(d_1)/(S\sigma\sqrt{T})$.

**Vega.** $\partial_\sigma C = S\varphi(d_1)\,(\partial_\sigma d_1 - \partial_\sigma d_2) = S\varphi(d_1)\sqrt{T}$, because $d_2 = d_1 - \sigma\sqrt{T}$.

**Theta and rho.** The same cancellation gives $\rho = KTe^{-rT}N(d_2)$; for theta use the PDE: $\Theta = rC - rS\Delta - \tfrac12\sigma^2S^2\Gamma = -rKe^{-rT}N(d_2) - \dfrac{S\varphi(d_1)\sigma}{2\sqrt{T}}$.

**The hedging P&L.** Hold the option, short $\Delta$ shares, finance at $r$. By [[ito-lemma|Itô's lemma]] with the *realised* move $dS$,

$$
d\Pi = \Theta\,dt + \tfrac12\Gamma\,(dS)^2 + (\text{financing}) .
$$

The PDE says $\Theta\,dt + \tfrac12\Gamma S^2\sigma_{\text{impl}}^2\,dt + (\text{financing}) = 0$: the position breaks even if the stock moves at implied volatility. Subtracting, with $(dS)^2 = S^2\sigma_{\text{real}}^2\,dt$,

$$
d\,\mathrm{P\&L} = \tfrac12\Gamma S^2\big(\sigma_{\text{real}}^2 - \sigma_{\text{impl}}^2\big)\,dt .
$$

Carrying each day's gain to expiry at the risk-free rate, $\mathrm{P\&L} = \int_0^T e^{r(T-t)}\,\tfrac12\Gamma_t S_t^2\big(\sigma_{\text{real},t}^2 - \sigma_{\text{impl}}^2\big)\,dt$. The gamma sits inside the integral: the P&L is path-dependent, and realised variance only counts near the strike.

## Assumptions & Edge Cases

- **Model dependence.** These are Black–Scholes Greeks at constant volatility. With a smile, delta depends on how implied vol is assumed to move with spot: "sticky strike" gives $\Delta_{BS}$, "sticky delta" $\Delta_{BS} + \nu\,\partial\sigma_{\text{impl}}/\partial S$.
...
- **Away from the money.** $\Gamma$ and $\nu$ vanish; the option becomes a forward ($\Delta \to 1$) or nothing ($\Delta \to 0$).
- **Where the peaks are.** Both peak near the forward $Ke^{-rT}$ but not together: gamma at $S = Ke^{-(r + \frac32\sigma^2)T}$, slightly below, vega at $S = Ke^{(\frac12\sigma^2 - r)T}$, slightly above. Both carry the same $\varphi(d_1)$; what separates them is the extra $1/S$ in gamma against the extra $S$ in vega.
- **Theta can be positive.** A deep in-the-money European put earns theta (the $rKe^{-rT}N(-d_2)$ term dominates): you are owed $K$ and get closer to receiving it. Same for a call on a high-yield stock.
- **Additivity.** Greeks are linear in the position, so a book's are the sum of its positions' — but only per underlying and, for vega, per expiry bucket.
- **The P&L formula is the gamma leg only.** It ignores discrete-hedging noise, transaction costs, and the vega P&L from a change in implied volatility.
...
- $\Gamma = 0.3948/(100 \times 0.25 \times 0.7071) = 0.0223$: a $1$ move changes delta by $0.022$, i.e. $223$ shares on the same book.
...
The daily break-even move follows from the trade-off: with $\Theta_\gamma = -6.98$ the gamma part of theta, $\tfrac12\Gamma(\delta S)^2 = -\Theta_\gamma\,\delta t$ gives $\delta S = S\sigma\sqrt{\delta t} = 100 \times 0.25 \times \sqrt{1/365} = 1.31$. Move more than $1.31$ in a day and long gamma pays for the theta.
...
The bumps match the closed forms to six decimals and the PDE residual is zero to machine precision. The gap between $-\tfrac12\sigma^2S^2\Gamma = -6.98$ and $\Theta = -7.94$ is the rate term $rS\Delta - rC = 0.02 \times (55.76 - 7.52) = 0.96$.

## Why It Matters in Quant Finance

- **Hedging is done in Greeks.** Delta with the underlying, gamma and vega with other options; a book is "flat" when its net Greeks are within limits, not when it holds nothing.
- **Risk limits are Greek limits.** Delta per underlying, vega per expiry bucket, gamma near expiries; breaching one forces a hedge whatever the trader's view.
- **P&L explain.** Each day's P&L splits into $\Delta\,\delta S + \tfrac12\Gamma\,\delta S^2 + \nu\,\delta\sigma + \Theta\,\delta t + \text{unexplained}$; a large unexplained term means the model or the data is wrong.
- **Volatility trading is gamma trading.** $\tfrac12\Gamma S^2(\sigma_{\text{real}}^2 - \sigma_{\text{impl}}^2)$ is the P&L of every delta-hedged option trade; variance swaps exist to remove the path dependence from $\Gamma_t$.
- **Delta–gamma VaR.** $\Delta\,\delta S + \tfrac12\Gamma\,\delta S^2 + \nu\,\delta\sigma$ approximates a book's move inside [[value-at-risk]] engines that cannot reprice every option per scenario.
- **Gamma is the Itô term.** The $\tfrac12\Gamma(dS)^2$ above is the second-order term of [[ito-lemma|Itô's lemma]]; theta is what the [[black-scholes]] PDE says you pay for it.
...
:::
::: answer
Slightly above. With $S = K$, $d_1 = (r + \tfrac12\sigma^2)\sqrt{T}/\sigma > 0$, so $\Delta = N(d_1) > 0.5$ — $0.558$ in the worked example. Two effects push it up: the forward is above spot ($r > 0$), and the $\tfrac12\sigma^2$ term of the lognormal. Delta is exactly $0.5$ at $S = Ke^{-(r + \sigma^2/2)T}$. The ATM put has delta $-0.442$: call and put deltas sum to $1$ in absolute value.
:::
:::
...
:::
::: answer
Per option: gamma gain $\tfrac12 \times 0.0223 \times 2^2 = 0.0446$; theta for one trading day $\approx \tfrac12\Gamma S^2\sigma^2/252 = \tfrac12 \times 0.0223 \times 10^4 \times 0.0625/252 = 0.0277$; net $+0.017$ per option, about $+170$ on the $10\,000$ shares. Faster: the break-even move is $\sigma/\sqrt{252} = 1.57\,\%$ and $2\,\%$ exceeds it, so long gamma wins by $\tfrac12\Gamma S^2(0.02^2 - 0.0157^2) \approx 0.017$ per option.
:::
:::
...
:::
::: answer
$\nu/\Gamma = S\varphi(d_1)\sqrt{T} \cdot S\sigma\sqrt{T}/\varphi(d_1) = \sigma S^2 T$. For one expiry a gamma-neutral book is vega-neutral and conversely. Across expiries $T$ differs: a long-dated option carries much vega per unit of gamma, a short-dated one the reverse. A calendar spread (long the 1-year, short the 1-month, same strike) is then long vega and short gamma at once: it gains if implied vol rises, loses if the stock moves a lot short term. No single vanilla trade isolates "volatility".
:::
:::
...
:::
::: answer
By Itô the hedged portfolio changes by $\Theta\,dt + \tfrac12\Gamma(dS)^2$ plus financing; the PDE makes this zero when $(dS)^2 = S^2\sigma_{\text{impl}}^2\,dt$, so the realised $(dS)^2 = S^2\sigma_{\text{real}}^2\,dt$ leaves $\tfrac12\Gamma S^2(\sigma_{\text{real}}^2 - \sigma_{\text{impl}}^2)\,dt$. Integrated, $\mathrm{P\&L} = \int_0^T \tfrac12\Gamma_t S_t^2(\sigma_{\text{real},t}^2 - \sigma_{\text{impl}}^2)\,dt$, with $\Gamma_t$ large near the strike and near zero far from it. If the stock moves at 40 % while 30 % away ($\Gamma_t \approx 0$) then sits at 10 % near the strike ($\Gamma_t$ large), averaging 25 %, the integrand is negative where it matters: the long-gamma trader loses although average realised vol matched implied. Variance swaps replicate a log contract whose dollar gamma $\Gamma S^2$ is constant, removing the weighting.
:::
:::
