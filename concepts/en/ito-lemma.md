...
Let $X_t$ be an Itô process, $dX_t = \mu_t\,dt + \sigma_t\,dW_t$ with $\mu_t, \sigma_t$ adapted, and let $f(t, x)$ be $C^1$ in $t$ and $C^2$ in $x$.
...
Compactly: $df = \partial_t f\,dt + \partial_x f\,dX_t + \tfrac12 \partial_{xx} f\,(dX_t)^2$, with $(dX_t)^2 = \sigma_t^2\,dt$ from the table. The rigorous statement is the integral form $f(T, X_T) = f(0, X_0) + \int_0^T (\cdots)\,dt + \int_0^T \sigma_t\,\partial_x f\,dW_t$, the last term an Itô integral: the integrand is evaluated at the left end of each interval.

For $X_t = (X^1_t, \dots, X^n_t)$ with $dX^i_t = \mu^i_t\,dt + \sum_j \sigma^{ij}_t\,dW^j_t$ and $d\langle W^j, W^k \rangle_t = \rho_{jk}\,dt$:
...
For two assets with volatilities $\sigma_1, \sigma_2$ and correlation $\rho$ the cross term is $d\langle X^1, X^2\rangle_t = \rho\,\sigma_1\sigma_2\,dt$. Taking $f(x, y) = xy$:
...
Ordinary calculus stops after two terms; the covariation $d\langle X, Y\rangle_t = \rho\,\sigma^X_t \sigma^Y_t\,dt$ is the Itô correction, and it vanishes when one process has finite variation, for instance $Y_t = e^{-rt}$.

## Derivation

Fix a partition $0 = t_0 < \dots < t_n = T$ with step $\Delta t$ and $\Delta X_k = \mu\,\Delta t + \sigma\,\Delta W_k$. Taylor to second order:
...
Expand $(\Delta X_k)^2 = \sigma^2 (\Delta W_k)^2 + 2\mu\sigma\,\Delta t\,\Delta W_k + \mu^2 (\Delta t)^2$ and sum. Three facts decide what survives as $\Delta t \to 0$:

1. $\sum_k (\Delta W_k)^2 \to T$ in $L^2$: $\mathbb{E}[(\Delta W_k)^2] = \Delta t$ and $\operatorname{Var}[(\Delta W_k)^2] = 2(\Delta t)^2$, so the sum has mean $T$ and variance $2T\,\Delta t \to 0$. This is the precise content of $(dW)^2 = dt$ — not only in expectation, but with vanishing variance.
2. $\sum_k \Delta t\,\Delta W_k$ has mean $0$ and variance $T(\Delta t)^2 \to 0$: the mixed term $dt\,dW$ dies.
3. $\sum_k (\Delta t)^2 = T\,\Delta t \to 0$, and higher-order terms vanish likewise.

The only second-order survivor is $\tfrac12 \sigma^2\,\partial_{xx} f\,\Delta t$, and the limit gives Itô's lemma; multiplicatively, $(dX)^2 = \sigma^2 dt$, $dt\,dX = 0$, $(dt)^2 = 0$.

**Where the left end point enters.** The first-order sum $\sum_k \partial_x f(t_k, X_{t_k})\,\Delta W_k$ evaluates the integrand at the *start* of each interval, so its limit $\int \partial_x f\,dW$ is a martingale: each term is a fair bet given the past. Evaluating at the midpoint gives the Stratonovich integral, for which the ordinary chain rule holds and the correction is absorbed into the integral. Finance uses Itô because a hedge chosen at $t_k$ can only use information available at $t_k$.

## Assumptions & Edge Cases

- **Smoothness.** $f$ must be $C^2$ in $x$. For $f(x) = |x|$, or a payoff $(x - K)^+$, the second derivative is a Dirac mass and Itô's lemma acquires a *local time* term (Tanaka's formula) — hence applying Itô to the smooth price function $V(t, S)$, never to the payoff.
...
- **Itô processes only.** Drift and volatility must be adapted and integrable ($\int_0^T \sigma_t^2\,dt < \infty$ almost surely); the SDE itself needs locally Lipschitz coefficients to have a solution.
...
- **Itô versus Stratonovich.** $\int_0^T W_t\,dW_t = \tfrac12 (W_T^2 - T)$ in the Itô sense but $\tfrac12 W_T^2$ in the Stratonovich sense. Always say which you mean: physics often uses Stratonovich, finance uses Itô.

## Worked Example

**1. Log of a geometric Brownian motion.** With $dS_t = \mu S_t\,dt + \sigma S_t\,dW_t$ and $f(x) = \ln x$:

$$
d\ln S_t = \frac{1}{S_t}\,dS_t - \frac{1}{2 S_t^2}\,(dS_t)^2 = \Big(\mu - \tfrac12 \sigma^2\Big)\,dt + \sigma\,dW_t ,
$$

hence $S_T = S_0 \exp\big((\mu - \tfrac12\sigma^2)T + \sigma W_T\big)$. The $-\tfrac12\sigma^2$ is the volatility drag: the median growth rate is below the mean rate $\mu$.
...
No $dt$ term: $M_t = e^{\lambda W_t - \lambda^2 t/2}$ is a martingale. It is the density in Girsanov's theorem, and with $\lambda = \sigma$ it shows that $e^{-rt}S_t$ is a martingale exactly when $\mu = r$ — the risk-neutral condition behind [[black-scholes]].

Numerically for application 2: form the left-point sum $2\sum_k W_{t_k}\,\Delta W_k$ and compare with $W_T^2$, with and without the correction $T$.
...
Ordinary calculus misses by almost exactly $T = 1$ on every path. The Itô version is off only by $\sum_k (\Delta W_k)^2 - T$, whose standard deviation $\sqrt{2T\,\Delta t} \approx 0.045$ matches the last line. The discrete identity $W_T^2 = \sum_k \big(2 W_{t_k}\Delta W_k + (\Delta W_k)^2\big)$ is exact; all the convergence happens in the quadratic-variation column.

## Why It Matters in Quant Finance

- **The Black–Scholes PDE is Itô plus a hedge.** Apply the lemma to $V(t, S_t)$, short $\partial_S V$ shares, and the $dW$ term cancels; the surviving $dt$ term must earn the risk-free rate. See [[black-scholes]].
...
- **Multi-asset and stochastic-volatility models.** Heston, basket options, quanto adjustments all rely on the multi-dimensional lemma, and the covariation term is where the quanto drift $-\rho\,\sigma_S\sigma_{FX}$ comes from.
...
$$
The volatility of $Y$ is $\sqrt{\sigma_1^2 + \sigma_2^2 - 2\rho\sigma_1\sigma_2}$, the volatility entering Margrabe's exchange-option formula. The two extra drift terms $\sigma_2^2 - \rho\sigma_1\sigma_2$ are pure Itô corrections: the ordinary quotient rule would miss them.
:::
:::
