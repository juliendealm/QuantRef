...
Probability is always relative to what you know. Before any dice are rolled, the chance that two dice sum to 8 is $5/36$. Tell me the first die shows a 5, and that chance becomes $1/6$: the universe of possible outcomes has shrunk to the six rows where the first die is 5, and exactly one of them sums to 8.

Conditioning is nothing more than **restricting the sample space** to the outcomes compatible with what you learned, then renormalising so the probabilities add up to 1 again.

In finance, "what you know" is the information set at time $t$, written $\mathcal{F}_t$. Every fair price, every forecast, every risk number is a conditional quantity: $\mathbb{E}[X \mid \mathcal{F}_t]$, not $\mathbb{E}[X]$.
...
Rearranged, this is the **multiplication rule** $\mathbb{P}(A \cap B) = \mathbb{P}(A \mid B)\,\mathbb{P}(B)$, which chains to any number of events:
...
For random variables, the same idea gives the **conditional expectation** $\mathbb{E}[X \mid Y]$, itself a random variable (a function of $Y$), with the **tower property**:
...
Start from a finite sample space $\Omega$ with probability measure $\mathbb{P}$. Learning that $B$ occurred rules out every $\omega \notin B$. The natural new measure on $B$ keeps the relative weights of the surviving outcomes and rescales them to sum to 1:

$$
\mathbb{P}_B(\omega) = \frac{\mathbb{P}(\omega)}{\mathbb{P}(B)} \quad \text{for } \omega \in B, \qquad 0 \text{ otherwise}.
$$

Summing over $\omega \in A$ gives $\mathbb{P}_B(A) = \mathbb{P}(A \cap B)/\mathbb{P}(B)$. This is the definition above; it is the unique choice that (i) assigns probability 1 to $B$ and (ii) preserves ratios $\mathbb{P}(\omega_1)/\mathbb{P}(\omega_2)$ inside $B$.

The law of total probability follows by writing $A = \bigcup_i (A \cap B_i)$ as a disjoint union and applying the multiplication rule to each piece.

## Assumptions & Edge Cases

- **Conditioning on a null event.** $\mathbb{P}(A \mid B)$ is undefined when $\mathbb{P}(B) = 0$. Conditioning on a continuous variable taking an exact value ($Y = y$) needs densities or the measure-theoretic definition; naive use produces the Borel–Kolmogorov paradox.
...
- **Independence is not pairwise-transitive.** $A \perp B$ and $B \perp C$ does not imply $A \perp C$; and pairwise independence does not imply mutual independence.
...
The conditioning event has $11$ outcomes (six with the first die 5, six with the second die 5, minus the double-counted $(5,5)$). Among them, the sum is 8 for $(5,3)$ and $(3,5)$, so the answer is $2/11 \approx 0.182$. Compare with conditioning on "the *first* die shows 5", where the answer is $1/6$: the wording of the information matters.
...
Conditioning in code is literally *filter, then average*: `total[mask]` is the restricted sample space.
...
:::
::: answer
The conditioning event $\{HH, HT, TH\}$ has probability $3/4$; $\{HH\}$ has probability $1/4$. So $\mathbb{P} = (1/4)/(3/4) = 1/3$, not $1/2$. The classic follow-up: "given the *first* coin is heads" gives $1/2$.
:::
:::
