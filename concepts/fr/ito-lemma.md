...
Le calcul différentiel ordinaire repose sur un fait : le long d'une trajectoire régulière, un petit déplacement $dx$ change $f$ de $f'(x)\,dx$, et le terme du second ordre $\tfrac12 f''(x)\,(dx)^2$ est négligeable parce que $(dx)^2 \sim (dt)^2$ tend vers zéro bien plus vite que $dt$.

Le mouvement brownien casse ce raisonnement. Sur un pas de longueur $dt$, l'accroissement $dW \sim \mathcal{N}(0, dt)$ a une taille typique $\sqrt{dt}$, donc $(dW)^2$ est d'ordre $dt$, le même ordre que le pas de temps lui-même. Sommer $(dW)^2$ sur les $n$ pas qui couvrent $[0, T]$ ne donne pas zéro : cela donne $T$. C'est la variation quadratique du [[brownian-motion|mouvement brownien]].

Le terme de Taylor du second ordre survit donc, et une fonction du mouvement brownien acquiert une dérive supplémentaire $\tfrac12 f''(W_t)\,dt$. Le lemme d'Itô, ce n'est rien d'autre que cela : **Taylor à l'ordre 2, remplacer $(dW)^2$ par $dt$, jeter tout ce qui est plus petit.**

Un signe concret que le calcul ordinaire oublie quelque chose : $W_t^2$ a pour espérance $t$, qui croît, alors que « $d(W^2) = 2W\,dW$ » aurait une dérive nulle. Cette croissance est exactement le terme d'Itô.

## Formulation mathématique

Les règles heuristiques, valables dans tout calcul à l'ordre $dt$ :
...
Soit $X_t$ un processus d'Itô, $dX_t = \mu_t\,dt + \sigma_t\,dW_t$ avec $\mu_t, \sigma_t$ adaptés, et soit $f(t, x)$ de classe $C^1$ en $t$ et $C^2$ en $x$. Alors :
...
Une façon compacte de le retenir : $df = \partial_t f\,dt + \partial_x f\,dX_t + \tfrac12 \partial_{xx} f\,(dX_t)^2$, avec $(dX_t)^2 = \sigma_t^2\,dt$ d'après la table de multiplication.

La forme intégrale est l'énoncé rigoureux : $f(T, X_T) = f(0, X_0) + \int_0^T (\cdots)\,dt + \int_0^T \sigma_t\,\partial_x f\,dW_t$, où le dernier terme est une intégrale d'Itô, c'est-à-dire que l'intégrande est évalué à l'extrémité gauche de chaque intervalle de temps.

**Plusieurs dimensions.** Pour $X_t = (X^1_t, \dots, X^n_t)$ avec $dX^i_t = \mu^i_t\,dt + \sum_j \sigma^{ij}_t\,dW^j_t$ et des mouvements browniens corrélés $d\langle W^j, W^k \rangle_t = \rho_{jk}\,dt$ :
...
Pour deux actifs de volatilités $\sigma_1, \sigma_2$ et de corrélation $\rho$, le terme croisé vaut simplement $d\langle X^1, X^2\rangle_t = \rho\,\sigma_1\sigma_2\,dt$.

**Règle du produit.** En prenant $f(x, y) = xy$ :
...
Le calcul ordinaire s'arrête après deux termes ; la covariation $d\langle X, Y\rangle_t = \rho\,\sigma^X_t \sigma^Y_t\,dt$ est la correction d'Itô. Elle disparaît quand l'un des deux processus est à variation finie, par exemple $Y_t = e^{-rt}$.

## Dérivation

Fixons une subdivision $0 = t_0 < t_1 < \dots < t_n = T$ de pas $\Delta t$ et écrivons $\Delta X_k = X_{t_{k+1}} - X_{t_k} = \mu\,\Delta t + \sigma\,\Delta W_k$. Taylor à l'ordre 2 :
...
Développons $(\Delta X_k)^2 = \sigma^2 (\Delta W_k)^2 + 2\mu\sigma\,\Delta t\,\Delta W_k + \mu^2 (\Delta t)^2$ et sommons sur $k$. Trois faits décident des sommes qui survivent quand $\Delta t \to 0$ :

1. $\sum_k (\Delta W_k)^2 \to T$ dans $L^2$. En effet $\mathbb{E}[(\Delta W_k)^2] = \Delta t$ et $\operatorname{Var}[(\Delta W_k)^2] = 2(\Delta t)^2$, donc $\mathbb{E}\big[\sum_k (\Delta W_k)^2\big] = T$ et $\operatorname{Var}\big[\sum_k (\Delta W_k)^2\big] = 2T\,\Delta t \to 0$. C'est le contenu précis de $(dW)^2 = dt$ : pas seulement en espérance, mais avec une variance qui s'annule.
2. $\sum_k \Delta t\,\Delta W_k$ est de moyenne $0$ et de variance $n\,(\Delta t)^3 = T(\Delta t)^2 \to 0$ : le terme mixte $dt\,dW$ meurt.
3. $\sum_k (\Delta t)^2 = T\,\Delta t \to 0$, et tous les termes d'ordre supérieur disparaissent de même.

Le seul survivant du second ordre est donc $\tfrac12 \sigma^2\,\partial_{xx} f\,\Delta t$, et le passage à la limite donne le lemme d'Itô. Écrit sous forme multiplicative : $(dX)^2 = \sigma^2 dt$, $dt\,dX = 0$, $(dt)^2 = 0$, ce qui est la table de multiplication.

**Où intervient l'extrémité gauche.** La somme du premier ordre $\sum_k \partial_x f(t_k, X_{t_k})\,\Delta W_k$ évalue l'intégrande au *début* de chaque intervalle. Sa limite est l'intégrale d'Itô $\int \partial_x f\,dW$, une martingale parce que chaque terme est un pari équitable sachant le passé. Évaluer au milieu de l'intervalle donne l'intégrale de Stratonovich, pour laquelle la règle de dérivation ordinaire reste valable et aucune correction n'apparaît : la correction a simplement été absorbée dans l'intégrale. La finance utilise Itô parce qu'une couverture choisie en $t_k$ ne peut utiliser que l'information disponible en $t_k$.

## Hypothèses et cas limites

- **Régularité.** $f$ doit être $C^2$ en $x$. Pour $f(x) = |x|$, ou pour un payoff d'option $(x - K)^+$, la dérivée seconde est une masse de Dirac et le lemme d'Itô acquiert un terme de *temps local* (formule de Tanaka). C'est pourquoi on applique Itô à la fonction *prix* $V(t, S)$, régulière, jamais directement au payoff.
...
- **Processus d'Itô seulement.** Dérive et volatilité doivent être adaptées et intégrables ($\int_0^T \sigma_t^2\,dt < \infty$ presque sûrement) ; l'EDS elle-même demande des conditions (coefficients localement lipschitziens) pour avoir une solution.
...
Trois applications à savoir faire au tableau.

**1. Logarithme d'un mouvement brownien géométrique.** Avec $dS_t = \mu S_t\,dt + \sigma S_t\,dW_t$ et $f(x) = \ln x$, $\partial_x f = 1/x$ et $\partial_{xx} f = -1/x^2$ :
...
Pas de terme en $dt$ : $M_t = e^{\lambda W_t - \lambda^2 t/2}$ est une martingale. C'est la densité utilisée dans le théorème de Girsanov, et avec $\lambda = \sigma$ elle montre que l'action actualisée $e^{-rt}S_t$ est une martingale exactement quand $\mu = r$, la condition risque-neutre derrière [[black-scholes]].

**Vérification numérique de l'application 2.** On simule des trajectoires, on forme la somme à gauche $2\sum_k W_{t_k}\,\Delta W_k$ et on la compare à $W_T^2$ avec et sans la correction $T$ :
...
Le calcul ordinaire se trompe de presque exactement $T = 1$ sur chaque trajectoire. La version d'Itô ne s'écarte que de $\sum_k (\Delta W_k)^2 - T$, dont l'écart-type $\sqrt{2T\,\Delta t} \approx 0{,}045$ correspond à la dernière ligne. En fait l'identité discrète $W_T^2 = \sum_k \big(2 W_{t_k}\Delta W_k + (\Delta W_k)^2\big)$ est exacte ; toute la convergence se joue dans la colonne de variation quadratique.
...
- **Modèles multi-actifs et à volatilité stochastique.** Heston, options sur panier, ajustements quanto : tous reposent sur le lemme multidimensionnel avec corrélation, et le terme de covariation est l'origine de la dérive quanto $-\rho\,\sigma_S\sigma_{FX}$.
...
$$
La volatilité de $Y$ vaut $\sqrt{\sigma_1^2 + \sigma_2^2 - 2\rho\sigma_1\sigma_2}$, qui est la volatilité qui entre dans la formule de Margrabe pour l'option d'échange. Les deux termes de dérive supplémentaires $\sigma_2^2 - \rho\sigma_1\sigma_2$ sont de pures corrections d'Itô : la règle du quotient ordinaire les manquerait.
:::
:::
