...
Une probabilité est toujours relative à ce que l'on sait. Avant de lancer deux dés, la chance que leur somme fasse 8 vaut $5/36$. Si l'on t'annonce que le premier dé montre 5, cette chance passe à $1/6$ : l'espace des possibles s'est réduit aux six cas où le premier dé vaut 5, et un seul d'entre eux donne 8.
...
Réécrite : la **règle de multiplication** $\mathbb{P}(A \cap B) = \mathbb{P}(A \mid B)\,\mathbb{P}(B)$, qui s'enchaîne pour un nombre quelconque d'événements :
...
Pour des variables aléatoires, la même idée donne l'**espérance conditionnelle** $\mathbb{E}[X \mid Y]$, elle-même une variable aléatoire, avec la **propriété de tour** :
...
Sur un $\Omega$ fini, apprendre que $B$ s'est produit élimine tout $\omega \notin B$ ; on conserve les poids relatifs des survivants et on les remet à l'échelle pour sommer à 1 :

$$
\mathbb{P}_B(\omega) = \frac{\mathbb{P}(\omega)}{\mathbb{P}(B)} \quad \text{pour } \omega \in B, \qquad 0 \text{ sinon}.
$$

En sommant sur $\omega \in A$, on obtient $\mathbb{P}_B(A) = \mathbb{P}(A \cap B)/\mathbb{P}(B)$ — l'unique mesure qui donne probabilité 1 à $B$ et conserve les rapports $\mathbb{P}(\omega_1)/\mathbb{P}(\omega_2)$ à l'intérieur de $B$.

Les probabilités totales s'obtiennent en écrivant $A = \bigcup_i (A \cap B_i)$ comme union disjointe et en appliquant la règle de multiplication à chaque morceau.

## Hypothèses et cas limites

- **Événement conditionnant de probabilité nulle.** $\mathbb{P}(A \mid B)$ n'est pas défini si $\mathbb{P}(B) = 0$. Conditionner par un $Y = y$ continu demande des densités ou la définition mesure-théorique ; un usage naïf mène au paradoxe de Borel–Kolmogorov.
...
- **L'indépendance n'est pas transitive.** $A \perp B$ et $B \perp C$ n'impliquent pas $A \perp C$ ; l'indépendance deux à deux n'implique pas l'indépendance mutuelle.
...
L'événement conditionnant compte $11$ issues (six avec le premier dé à 5, six avec le second, moins le doublon $(5,5)$). La somme vaut 8 pour $(5,3)$ et $(3,5)$, donc la réponse est $2/11 \approx 0{,}182$ — contre $1/6$ en conditionnant par « le *premier* dé montre 5 ». La formulation de l'information change tout.
...
- **Un prix est une espérance conditionnelle.** Sous la mesure risque-neutre, le prix en $t$ d'un flux $H$ vaut $e^{-r(T-t)}\,\mathbb{E}^{\mathbb{Q}}[H \mid \mathcal{F}_t]$ ; une [[martingales|martingale]] se définit entièrement par de telles espérances. Voir [[black-scholes]].
- **Un signal est un avantage conditionnel.** Un signal est utile si $\mathbb{E}[r_{t+1} \mid \text{signal}_t] \neq \mathbb{E}[r_{t+1}]$ ; un backtest estime exactement cette moyenne conditionnelle.
- **Le risque est conditionnel.** L'expected shortfall vaut $\mathbb{E}[L \mid L \ge \mathrm{VaR}]$ (voir [[value-at-risk]]) ; un stress test est une espérance sachant un scénario.
...
:::
::: answer
$\{PP, PF, FP\}$ a probabilité $3/4$ et $\{PP\}$ a $1/4$, donc $\mathbb{P} = (1/4)/(3/4) = 1/3$, et non $1/2$. La relance classique, « sachant que la *première* pièce montre pile », donne $1/2$.
:::
:::
...
:::
::: answer
Pas en général. Soit $A$ = « la stratégie 1 a gagné » et $B$ = « la stratégie 2 a gagné », indépendants, et conditionne par $C$ = « exactement une des deux a gagné ». Savoir que $A$ s'est produit implique alors que $B$ ne s'est pas produit : dépendance négative parfaite sachant $C$. C'est un biais de collision — conditionner par une conséquence commune crée une dépendance. Le paradoxe de Berkson est le même mécanisme avec $C = A \cup B$. Cela apparaît dès qu'on analyse un échantillon *sélectionné*, par exemple les fonds qui ont survécu.
:::
:::
